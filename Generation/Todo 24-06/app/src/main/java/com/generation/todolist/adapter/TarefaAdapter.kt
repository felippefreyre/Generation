...
class TarefaAdapter(
    ...
    override fun onBindViewHolder(holder: TarefaViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener{
            taskClickListener.onToskCliclListener(tarefa)
        }
        holder.binding.switchAtivo
            setOnClickListener {compoundButton, ativo->
             tarefa.status = ativo
                mainViewModel.updateTarefa(tarefa)
            }

        holder.binding.buttonDeletar.setOnClickListener(){
          showAlertDialogo(tarefa.id)
        }
    }
...
private fun showAlertDialogo(id: Long)}
   AlertDialog.Builder(context)
       .setTitle("Excluir")
       setMessage("Deseja Exluir e Tarefa? ")
       .setPositivaButton("Sim"){
